...
MB_extract <- function(plot_type, nGroups, image_file, image_details, variable, calpoints, point_vals,entered_N,...){
	...
	for(i in 1:nGroups) {
		...
		while(add_removeQ=="r") {
			...
			internal_redraw(image_file=image_file, image_details=image_details, plot_type=plot_type, variable=variable, calpoints=calpoints, point_vals=point_vals, raw_data=raw_data)
			...
			if(add_removeQ=="r") {
				internal_redraw(image_file=image_file, image_details=image_details, plot_type=plot_type, variable=variable, calpoints=calpoints, point_vals=point_vals, raw_data=raw_data[-rows,])
				raw_data[rows,"id"] <- NA
			}		
		}
	}
