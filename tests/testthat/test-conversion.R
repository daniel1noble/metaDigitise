...
test_that("Checking calibrate..", {
	expect_equal(
		calibrate(raw_data = data.frame(x=50,y=50), calpoints = data.frame(x=c(0,0,0,100),y=c(0,100,0,0)), point_vals = c(1,2,3,4)), 
		data.frame(x=3.5,y=1.5), 
		info = "calibrate failed")
	
	expect_equal(
		calibrate(raw_data = data.frame(x=50,y=50), calpoints = data.frame(x=c(0,0),y=c(0,100)), point_vals = c(1,2)), 
...
test_that("Checking process_data..", {
	expect_equal(
		process_data(object=list(plot_type="mean_error", variable="y", calpoints = data.frame(x=c(0,0),y=c(0,100)), point_vals = c(1,2), raw_data = data.frame(id=rep("control",2), x=c(60,60), y=c(75,50), n=rep(20,2)))), 
		data.frame(id=as.factor("control"),mean=1.5, error=0.25, n=20, variable="y", stringsAsFactors = FALSE), 
		info = "process_data failed")

	expect_equal(
		process_data(object=list(plot_type="scatterplot", variable=c(x="x",y="y"), calpoints = data.frame(x=c(0,0,0,100),y=c(0,100,0,0)),point_vals = c(1,3,2,4), raw_data=data.frame(id=1,x=c(25,75),y=c(50,50)))), 
