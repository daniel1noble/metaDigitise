...
2. Extracting summary statistics from figures can be a slow process that is not easily reproducible. Additionally, current software lacks an ability to incorporate important meta-data (e.g., sample sizes, treatment names) about experiments. Furthermore, extraction software is often independent of analytical software preventing the automatic calculation of effect size statistics and making it more challenging to implement user defined analytical pipelines.
...
4. Digitised data can be easily re-plotted and checked, facilitating reproducible data extraction from plots with little inter-observer bias. Such approaches allow collaborative extraction of data. We hope that by making the process of figure extraction more flexible and easy to conduct it will improve transparency and quality of meta-analyses in the future.
%% 230 words
