...
For each bar a midpoint (mean x coordinates) and a frequency (mean y coordinates, rounded to the nearest integer) is calculated. The sample size is calculated as the sum of the frequencies., the mean as
\begin{equation}
\sigma = SE * \sqrt{n}
\end{equation}
and from 95\% confidence intervals as
...
Using the x coordinates, \fct{metaDigitise} then calculates the midpoint and the frequency for each bar. 
