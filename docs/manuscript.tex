...
%% 2- adding metadata
Secondly, digitising programs do not easily allow the integration of metadata at the time of data extraction, such as grouping points or adding variable names, group names and sample sizes. This makes the downstream calculations more laborious, as the information has to be added later, in most cases using different software.
...
For each bar a midpoint (m; mean x coordinates) and a frequency (f; mean y coordinates, rounded to the nearest integer) is calculated. The sample size, mean and SD were calculated as:
\begin{equation}
n = \sum_{i=1}^n{f_i}
\end{equation}
...
Assume that the user would like to extract summary statistics from studies measuring sepal length or width in iris species for a fictitious project. There are a few studies that only present these data in figures. As the user readds papers found from a systematic search, they add figures with relevant data to a "FiguresToExtract" folder as follows
...
Often, plots might contain multiple groups that the meta-analyst wants to extract from. \fct{metaDigitise} handles this nicely by prompting the user to enter the group first, followed by the digitisation of this groups data. After digitising the first group, and having exited (e.g., hit `esc' on mac from plot window), \fct{metaDigitise} will ask the user whether they would like to add another group. Users can continually add groups (\code{a}), delete groups (\code{d}), edit group (\code{e}) or finish plot and continue to the next one (\code{f} - if another plot exists). 
...
\subsection{Checking Extraction}

\subsection{Post-hoc adding smaple sizes}
...
Although \pkg{metaDigitise} is already very flexible, and provides functionality not seen in any other package, from comparison with other packages (Table \ref{tab:comparison}) it is clear that there are some functions that it does not perform. A notable feature that \pkg{metaDigitise} lacks is automated point detection. This is available in several packages (Table \ref{tab:comparison}). However, from our experience of using these functions, as calibration (for point detection) needs to be done for each plot individually, and often some points are not detected, it is much more reliable, and sometimes even quicker to manually select the points. From the testing of our package (see above) figures can be extracted in around 1-2 minutes, including entry of metadata. We do not think that current automation methods give substantial benefits to this. 

Another feature that \pkg{metaDigitise} (currently) lacks, if a zoom function. This may enable users to gain greater accuracy when clicking on points. However, from our own experience, if you are using a reasonably sized screen then the accuracy is already high from these programs (see results above for example), and not much gain is to be had from zooming in on points.

\pkg{metaDigitise} also does not extract lines from figures, as many other packages do. In our own experience, this is not particularly useful for meta-analysis, although we recognise that it may be useful in other fields. In this case we would recommend extracting data as a scatter plot, and clicking along the line in question. A model can then be fitted to these points to estimate the parameters needed.

Finally, \pkg{metaDigitise} does not allow for asymmetric error bars. At present this is a deliberate omission, as it is not clear how best to derive SD from such data, given also that such asymmetric error bars may represent different things in different figures. 
